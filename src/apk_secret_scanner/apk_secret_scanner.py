...
class APKSecretScanner:
    def __init__(self,
                 decompiler_kwargs: dict = {},
                 scanner_kwargs: dict = {},
                 output_file: Optional[Path] = None,
                 output_format: str = "json",
                 groupby: Literal["file", "locator", "both"] = "file",
                 cleanup: bool = True,
                 ):
        # workers
        self.decompiler = JavaDecompiler(**decompiler_kwargs)
    ...
    def print_status(self, end='\r'):
        ...
        if self.num_scanning:
            status_message += f"Scanned: {self.num_scanned}/{self.num_scanning} ({self.num_scanned/self.num_scanning*100:3f}%) scanned. | "
    ...
    def decompiled_files_generator(self, file_paths: Iterable[Path]) -> Generator[Path, None, None]:
        for file_path, output_dir, success in self.decompiler.decompile_concurrently(file_paths):
            if not self.decompile_start_time:
                self.decompile_start_time = datetime.now()
                print(f"\nDecompiling {self.num_files} files...")
                print(f"Started at {self.decompile_start_time.strftime('%H:%M:%S:%SS')}")
                self.print_status('\n')
            ...
            if success:
                ...
                self.decompile_results[file_path] = (output_dir, success)
        ...
        if self.decompile_start_time:
            ...
            print(f"\nDecompiled {self.num_decompiled} files with {self.num_decompile_errors} errors. Elapsed time: {self.decompile_elapsed_time}")
    ...
    def scan_secret_results_generator(self, file_paths: Generator[Path, None, None]) -> Generator[SecretResult, None, None]:
        ...
        for file_path, file_secret_results in self.secret_scanner.scan_concurrently(file_paths):
            if not self.scan_start_time:
                ...
                print(f"\nScanning files...")
                print(f"Started at {self.scan_start_time.strftime('%H:%M:%S:%f')}")
        ...
        self.print_status('\n')
        print(f"\nScanned {self.num_scanned} files with {self.num_secrets} secrets found. Elapsed time: {self.scan_elapsed_time}")

    def decompile_and_scan(self, file_paths: list[Path]) -> list[SecretResult]:
        ...
        for secret_result in self.scan_secret_results_generator(self.decompiled_files_generator(file_paths)):
            self.num_secrets += 1
            self.secrets_results.append(secret_result)
            if secret_result not in self.unique_secrets:
                self.num_unique_secrets += 1
                self.print_secret_found(secret_result)
                self.unique_secrets.add(secret_result)
            self.print_status()

        self.total_elapsed_time = datetime.now() - self.decompile_and_scan_start_time
        print(f"\nCOMPLETE\nFound \033[92m{self.num_secrets - num_secrets_before} new secrets\033[0m ({self.num_unique_secrets - num_unique_secrets_before} unique).")
        print(f"Decompiled {self.num_decompiled} files with {self.num_decompile_errors} errors in {self.decompile_elapsed_time}.")
        print(f"Scanned {self.num_scanned} files with {self.num_secrets} secrets found. in {self.scan_elapsed_time}.")
        print(f"Total Elapsed time: {datetime.now() - self.decompile_and_scan_start_time}")

        self.write_output(file_paths)
        return self.secrets_results

    def make_secret_result_serializable(self, secret_result: SecretResult) -> dict:
        secret_result_dict = secret_result.model_dump()
        try:
            secret_result_dict["locator"] = secret_result_dict["locator"]["name"]
            secret_result_dict["file_path"] = str(secret_result_dict["file_path"])
            secret_result_dict["matched_pattern"] = str(secret_result_dict["matched_pattern"].pattern)
    ...
    def group_results_by_locator(self) -> dict[str, list[SecretResult]]:
        results_by_locator = {}
        for secret_result in self.unique_secrets:
    ...
    def group_results_by_input_file(self, file_paths: list[Path]) -> dict[str, list[SecretResult]]:
        results_by_input_file = {}
        for secret_result in self.unique_secrets:
            for file_path in file_paths:
                if str(file_path.name) in str(secret_result.file_path):
                    serializable_secret_result = self.make_secret_result_serializable(secret_result)
                    results_by_input_file.setdefault(str(file_path), []).append(serializable_secret_result)
                    break
        return results_by_input_file

    def write_output(self, file_paths:list[Path]):
        if self.groupby == "file":
            results = self.group_results_by_input_file(file_paths)
        elif self.groupby == "locator":
            results = self.group_results_by_locator()
        elif self.groupby == "both":
            results = {
                "by_file": self.group_results_by_input_file(file_paths),
    ...
    def __del__(self):
        if getattr(self, "cleanup", False):
            ...
            self.decompiler.concurrent_executor.shutdown()
            self.secret_scanner.concurrent_executor.shutdown()
